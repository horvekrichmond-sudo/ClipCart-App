...
const Sidebar = ({ activeTab, setActiveTab, isCollapsed = false }: SidebarProps) => {
  const primaryLinks = [
  ...
  return (
    <aside className="flex flex-col w-full h-full bg-white dark:bg-yt-dark overflow-y-auto scrollbar-hide py-6 px-3 transition-all duration-300">
      <div className="space-y-1">
        {primaryLinks.map(({ id, label, icon: Icon }) => (
          <button
            ...
            className={`w-full flex items-center transition-all overflow-hidden ${
              isCollapsed ? 'justify-center px-0 py-3 rounded-2xl' : 'gap-4 px-4 py-3 rounded-xl'
            } ${
              activeTab === id 
          ...
          >
            <Icon 
              size={22} 
              className={`flex-shrink-0 ${activeTab === id ? 'text-yt-textLight dark:text-yt-textDark' : ''}`} 
      ...
      <div className={`mt-8 pt-8 border-t border-zinc-200 dark:border-zinc-800 space-y-1 ${isCollapsed ? 'flex flex-col items-center' : ''}`}>
        ...
        {secondaryLinks.map(({ id, label, icon: Icon }) => (
          <button
            key={label}
            title={isCollapsed ? label : undefined}
            className={`w-full flex items-center transition-all overflow-hidden ${
      ...
      <div className="mt-auto pt-8 border-t border-zinc-200 dark:border-zinc-800">
        ...
        >
          <div className={`flex-shrink-0 rounded-full flex items-center justify-center text-[10px] font-bold text-black border-2 overflow-hidden shadow-sm transition-all ${
            isCollapsed ? 'w-12 h-12' : 'w-10 h-10'
          } ${
            activeTab === 'profile' ? 'border-accent scale-105' : 'border-transparent'
          }`}>
